...
export default function NumeroALetras(num) {
  ...
  const centavo = data.centavos.toString();
 
  //   console.log(`0${data.centavos}`, data.enteros);
  ...
  return `${Millones(data.enteros)}${data.letrasMonedaPlural} ${centavo.length === 1 ? ` 0${centavo}` : centavo}/100`;
}
